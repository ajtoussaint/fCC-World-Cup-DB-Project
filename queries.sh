...
echo "$($PSQL "(SELECT t1.name FROM teams AS t1 FULL JOIN games ON t1.team_id=games.winner_id FULL JOIN teams AS t2 ON t2.team_id=games.opponent_id WHERE round='Eighth-Final' AND year=2014) UNION (SELECT t2.name FROM teams AS t1 FULL JOIN games ON t1.team_id=games.winner_id FULL JOIN teams AS t2 ON t2.team_id=games.opponent_id WHERE round='Eighth-Final' AND year=2014);")"
